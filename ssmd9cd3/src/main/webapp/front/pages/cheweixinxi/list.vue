	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.cheweihao" type="text" placeholder="车位号" ></input>
			</view>
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.cheweizhuangtai" type="text" placeholder="车位状态" ></input>
			</view>
			<view class="action">
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
								                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.cheweihao}}</view>
								                				                				                				                				                                <view class="image-view">
                    <image :style='{"borderRadius":"8rpx","width":"100%","height":"100%"}' mode="aspectFill" class="uni-product-image" :src="product.cheweitupian?product.cheweitupian.split(',')[0]:''"></image>
                </view>
                				                												<view style="display: flex;justify-content: space-between;">
					<text v-if="isAuth('cheweixinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('cheweixinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button v-if="isAuth('cheweixinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.cheweihao="";
				this.searchForm.cheweizhuangtai="";
			},
			//类别搜索
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('cheweixinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
												...
												if(this.searchForm.cheweihao){
					searchForm['cheweihao'] = '%' + this.searchForm.cheweihao + '%'
				}	
																if(this.searchForm.cheweizhuangtai){
					searchForm['cheweizhuangtai'] = '%' + this.searchForm.cheweizhuangtai + '%'
				}	
																																																				let res = await this.$api.list(`cheweixinxi`, searchForm);
				// 如果是第一页数据置空
